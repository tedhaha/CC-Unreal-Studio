...
| Metric | PASS criteria |
|---|---|
| **E1 — Version-aware** | References pinned UE version from `docs/engine-reference/unreal/` before suggesting API calls; flags post-cutoff risk |
| **E2 — File routing** | Routes file types to the correct sub-specialist (e.g., GAS assets `GA_*` / `GE_*` / `AS_*` → ue-gas-specialist, `WBP_*` → ue-umg-specialist, replicated systems → ue-replication-specialist) |
| **E3 — Unreal-specific patterns** | Enforces UE5 idioms (BP/C++ boundary, `UPROPERTY` exposure, GAS attribute rules, server-authoritative replication, CommonUI input layers) |
