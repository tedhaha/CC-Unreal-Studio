...
| PC (Screen Reader) | JAWS / NVDA / Windows Narrator | [Menu navigation announcements] | Not Started | Requires UMG widgets to expose accessible names and roles. UE 5.x ships the Slate Accessibility / Screen Reader plugin (`SlateAccessibility`) — verify the project enables it and that each widget sets `AccessibleSummaryBehavior` and `AccessibleText`. See `docs/engine-reference/unreal/`. |
...
|----------|-------|----------|-----------|
| [Does UE 5.x SlateAccessibility support dynamic widget updates for HUD elements, or only static menus?] | [ux-designer] | [Before Technical Setup gate] | [Unresolved — check `docs/engine-reference/unreal/`] |
